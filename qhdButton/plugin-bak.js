(function()
{
	...
	//获取按钮（ul）标签
	function getBtnA( editor, element )
	{
		while ( element ){
			var name = element.getName(),
				className = element.getAttribute('class')
	...
	CKEDITOR.plugins.qhdButton =
	{
		...
		init : function( editor )
		{
			...
			{				
				...
				if ( selection.getType() == CKEDITOR.SELECTION_ELEMENT )
				{
					...
					if ( selectedElement.is( 'a' ) ){
						buttonElement = selectedElement;	
					}
					if( selectedElement.is( 'span' ) ){
						buttonElement = selectedElement.getParent();	
					}			
				} */ 
			...
			{
				alert(1);
				editor.getCommand('qhdButton').setState(CKEDITOR.TRISTATE_OFF);
				var element = e.data.selection.getStartElement();	//获取当前选择的标签
				while ( element )
				{
					var className = element.getAttribute('class');
					// alert( className )
					// if( className == null ) { alert(1); break; }
					alert( element.is('a') );
					alert( className != null );
					alert( className.search(/btn/i) !=-1 );
					alert( element.is('a') && className != null && className.search(/btn/i) !=-1 )
					
					if ( element.is('a') && className != null && className.search(/btn/i) !=-1 ){					
						editor.removeMenuItem('link');	//删除右键菜单
						editor.removeMenuItem('unlink');	//删除右键菜单
						editor.getCommand('link').setState(CKEDITOR.TRISTATE_OFF);	//取消链接的当前状态
						editor.getCommand('qhdButton').setState(CKEDITOR.TRISTATE_ON);		//设置自定义列表的当前状态
					}
					element = element.getParent();
				}
				return null;
